...
public final class RtsStorageTickService {
    ...
    /**
     * 基于总槽位数计算初始刷新率（ticks/次）。
     * 以标准箱子（27 槽位）为基准分档，避免过度设计的对数公式。
     *
     * <table>
     *   <tr><th>槽位数</th><th>速率</th><th>含义</th></tr>
     *   <tr><td>≤ 27</td><td>1</td><td>每个 tick 刷新（1 个箱子）</td></tr>
     *   <tr><td>≤ 135</td><td>2</td><td>每 2 tick（5 个箱子）</td></tr>
     *   <tr><td>≤ 270</td><td>3</td><td>每 3 tick（10 个箱子）</td></tr>
     *   <tr><td>≤ 540</td><td>4</td><td>每 4 tick（20 个箱子）</td></tr>
     *   <tr><td>≤ 1080</td><td>5</td><td>每 5 tick（40 个箱子）</td></tr>
     *   <tr><td>≤ 2160</td><td>6</td><td>每 6 tick（80 个箱子）</td></tr>
     *   <tr><td>≤ 4320</td><td>7</td><td>每 7 tick（160 个箱子）</td></tr>
     *   <tr><td>＞ 4320</td><td>8</td><td>每 8 tick（超大规模）</td></tr>
     * </table>
     */
    private static int calculateInitialRate(List<IItemHandler> handlers) {
        ...
        int rate = 1;
        if (totalSlots > 27) rate = 2;
        if (totalSlots > 135) rate = 3;
        if (totalSlots > 270) rate = 4;
        if (totalSlots > 540) rate = 5;
        if (totalSlots > 1080) rate = 6;
        if (totalSlots > 2160) rate = 7;
        if (totalSlots > 4320) rate = 8;
        return rate;
    }
